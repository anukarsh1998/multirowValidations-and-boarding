           ...
           numberOfRows = request.body.bill_no.length;           
            for(let i=0; i< numberOfRows ; i++)
            {
              const schema = joi.object({
               ...
               let result = schema.validate({bill_no:request.body.bill_no[i],amount:request.body.amount[i],bill_date:request.body.bill_date[i],imgpath:request.body.imgpath[i]})
               if(result.error)
               {
                 console.log('ejssssss VAlidation'+JSON.stringify(result.error));
                 response.send(result.error.details[0].context.label);
    ...
    if(typeof(request.body.from) == 'object')
    {
        ...
        for(let i=0; i<numberOfRows ; i++)
        {
          const schema=joi.object({
           ...
           let result=schema.validate({activity_code:body.activity_code[i],to:body.to[i],from:body.from[i],purposeoftravel:body.purposeoftravel[i],amount:body.amount[i],kmtravelled:body.kmtravelled[i],imgpath:body.imgpath[i]});
           console.log('sdjabsdjb'+JSON.stringify(result));
           if(result.error)
           {
            console.log(' VAlidation'+JSON.stringify(result.error));
